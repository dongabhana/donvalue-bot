...
def item(row,due,ident_override=None):
    ident,cat,topic,hook,thr,a,b,calc,verdict,note,fmt,photo,_sha=row
    ...
    safe_verdict=re.sub(r'\d[\d,.]*\s*만?원','해당 금액',verdict)
...
def validate(x):
    ...
    if not x['verification']['real_world_price_claim']:
        assert re.search(r'\d[\d,.]*\s*만?원',x['caption']) is None
