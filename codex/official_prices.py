"""Fail closed when a referenced official price or policy source can no longer be confirmed."""
...
ALLOWED_HOSTS={'support.apple.com','www.apple.com','www.microsoft.com','store.nintendo.co.kr',
               'news.seoul.go.kr','www.customs.go.kr'}
...
def check_prices(item,session=requests):
    ...
    for source in item['verification']['sources']:
        ...
        if parsed.scheme!='https' or parsed.hostname not in ALLOWED_HOSTS:
            raise RuntimeError('Unexpected official price/policy source')
        try:
            response=session.get(url,timeout=25,headers={'User-Agent':'Mozilla/5.0 donvalue-price-check'},allow_redirects=False)
        except requests.RequestException:
            raise RuntimeError('Official source temporarily unavailable') from None
        if response.status_code!=200 or not matches_official_page(source,response.text):
            raise RuntimeError('Official source unavailable or changed; revised content required')
